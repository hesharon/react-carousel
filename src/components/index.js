...
const Carousel = ({ children, id }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('resize', setImageWidth, { passive: false })
    return () => {
  ...
  const carouselAction = track.action({
    action: 'User Action',
    label: 'carousel action',
    description: 'User used the carousel, they swiped or clicked an arrow'
  })
  ...
  return (
    <div className="carousel-main">
      <div
        ...
        {!mobileAndTabletCheck(navigator.userAgent || navigator.vendor || window.opera) && (
          <div>
            <button className="back move" onClick={() => transitionTo(currentIndex - 1, 0.5)}>
              <img src="/i/icons/chevron-left.svg" width="10px" />
            </button>
            <button className="next move" onClick={() => transitionTo(currentIndex + 1, 0.5)}>
              <img src="/i/icons/chevron-right.svg" width="10px" />
            </button>
          </div>
        )}
      </div>
